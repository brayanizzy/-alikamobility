...
const MemberCardCreatePage = () => {
  ...
  const handleSubmit = async (data) => {
    setSubmitting(true);
    try {
      await pb.collection('member_cards').create(data, { $autoCancel: false });
      toast.success('Carte membre créée avec succès !');
          ...
          <div className="max-w-2xl mx-auto">
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="bg-card border border-border rounded-2xl p-6 md:p-8 shadow-card">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-foreground">Ajouter une Carte Membre</h1>
                  <p className="text-sm text-muted-foreground">Créer une nouvelle carte membre avec QR code.</p>
                </div>
              </div>
              <MemberCardForm onSubmit={handleSubmit} submitLabel="Créer la carte" submitting={submitting} />
            </motion.div>
          </div>
        </main>
      </div>
...
export default MemberCardCreatePage;
