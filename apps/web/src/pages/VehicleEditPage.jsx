...
const VehicleEditPage = () => {
  ...
  const handleSubmit = async (data) => {
    setSubmitting(true);
    try {
      await pb.collection('vehicles').update(id, data, { $autoCancel: false });
      ...
      <div className="flex-1 flex">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8">
          <div className="max-w-2xl mx-auto">
            <button onClick={() => navigate(`/vehicles/${id}`)}
              className="flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors mb-6">
              <ArrowLeft className="w-4 h-4" /> Retour au véhicule
            </button>
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="bg-card border border-border rounded-2xl p-6 md:p-8 shadow-card">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-foreground">Modifier le Véhicule</h1>
                  <p className="text-sm text-muted-foreground">{vehicle.plate || vehicle.moto_number || vehicle.id}</p>
                </div>
              </div>

              <VehicleForm initialData={vehicle} onSubmit={handleSubmit} submitLabel="Enregistrer les Modifications" submitting={submitting} />
            </motion.div>
          </div>
        </main>
      </div>
