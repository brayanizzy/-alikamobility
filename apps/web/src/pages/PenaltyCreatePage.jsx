...
const PenaltyCreatePage = () => {
  ...
  const handleSubmit = async (data) => {
    setSubmitting(true);
    try {
      await pb.collection('penalties').create(data, { $autoCancel: false });
          ...
          <div className="max-w-2xl mx-auto">
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="bg-card border border-border rounded-2xl p-6 md:p-8 shadow-card">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-foreground">Ajouter une Pénalité</h1>
                  <p className="text-sm text-muted-foreground">Enregistrer une nouvelle pénalité.</p>
                </div>
              </div>
              <PenaltyForm onSubmit={handleSubmit} submitLabel="Créer la Pénalité" submitting={submitting} />
            </motion.div>
          </div>
        </main>
      </div>
